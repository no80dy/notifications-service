...
if __name__ == '__main__':
    ...
    release_films_query = monthly_release_films_sql.format(datetime.utcnow().replace(month=month_ago).strftime("%Y-%m-%d %H:%M:%S"))
    results_release_films_query = NotificationHandler.get_data_from_clickhouse(release_films_query)
    for result in results_release_films_query:
        release_film_ids = [v[0] for v in result]

    users_history_query = users_history.format(datetime.utcnow().replace(month=month_ago).strftime("%Y-%m-%d %H:%M:%S"))
    results_users_history_query = NotificationHandler.get_data_from_clickhouse(users_history_query)
